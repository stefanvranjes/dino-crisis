...
public class Coprocessor
{
    ...
    public static void ExecuteGPL(byte shift, bool lm)
    {
        TruncateAndSetMACAndIR(accumulator.ir1 * accumulator.ir0 + ((long)mathsAccumulator.mac1 << shift), 1, shift, lm);
        TruncateAndSetMACAndIR(accumulator.ir2 * accumulator.ir0 + ((long)mathsAccumulator.mac2 << shift), 2, shift, lm);
        TruncateAndSetMACAndIR(accumulator.ir3 * accumulator.ir0 + ((long)mathsAccumulator.mac3 << shift), 3, shift, lm);
